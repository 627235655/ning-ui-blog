...
var db = require('./db');
var co = require('co');
var { CLIENT, ALI_OSS } = require('./private-constants'); // 这里不方便暴露自己的信息， 配置可看以下注释
...
api.post('/api/addArticle', function (req, res) {
	...
	DB.find(data, function (err, docs) {
		...
		if (docs.length > 0) {
			res.json({
				status: 500,
				message: '当前已存在【' + data.articleName + '】文章',
			});
		} else {
			insertData(data);
		}
