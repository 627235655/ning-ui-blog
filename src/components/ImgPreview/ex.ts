...
export class ImagePreview {
	...
	constructor(
		...
		this.imgContainer = this.ref.querySelector(
			`.${this.prefix}imgContainer`
		);
		this.imgItems = this.imgContainer.querySelectorAll(
			`.${this.prefix}item`
		);
	...
	pcInitial() {
		...
		this.ref
			.querySelector(`.${this.prefix}close`)
	...
	mobileInitial() {
		...
		this.ref
			.querySelector(`.${this.prefix}close`)
			.addEventListener('touchstart', this.close.bind(this));
	}
	bindTrigger() {
		let images: Array<string> = [];
		let triggerItems: NodeListOf<HTMLElement> = document.querySelectorAll(
			...
			if (img.complete) {
				...
				img.onload = (function(el) {
					return function() {
						...
						styleObj = el.getBoundingClientRect();
						const top: number =
							(imgContainerHeight - styleObj.height) / 2;
						const left: number =
							(imgContainerWidth - styleObj.width) / 2;
				...
				img.onerror = (function(el) {
					return function(e: Event) {
						...
						(<HTMLImageElement>e.currentTarget).alt =
							'图片加载错误';
					};
				})(el).bind(this);
			}
	...
	handleClick(e?: TouchEvent & MouseEvent) {
		let close: HTMLElement = <HTMLElement>(
			this.ref.querySelector(`.${this.prefix}close`)
		);
		let bottom: HTMLElement = <HTMLElement>(
			this.ref.querySelector(`.${this.prefix}bottom`)
		);
	...
	handleDoubleClick(e: TouchEvent & MouseEvent) {
		...
		if (
			Math.abs(rotateDeg % 360) == 90 ||
			Math.abs(rotateDeg % 360) == 270
		) {
		...
		if (isBigSize) {
			//当前浏览元素为大尺寸时执行缩小操作，小尺寸执行放大操作
			switch (Math.abs(rotateDeg % 360)) {
				case 0:
				case 180:
					scaleX =
						Number(curItem.dataset.initialWidth) / curItemWidth;
					scaleY =
						Number(curItem.dataset.initialHeight) / curItemHeight;
					break;
				case 90:
				case 270:
					scaleX =
						Number(curItem.dataset.initialWidth) / curItemHeight;
					scaleY =
						Number(curItem.dataset.initialHeight) / curItemWidth;
					break;
				default:
					break;
			}
			...
			case 0:
				curItem.style.cssText = `;
                    ...
                    transform: 
                        ...
                        translateY(${(-(mouseY - curItemViewTop - centerY) *
							(scaleY - 1)) /
							scaleY}px) 
                        translateX(${(-(mouseX - curItemViewLeft - centerX) *
							(scaleX - 1)) /
							scaleX}px) 
                    ;
                `;
			...
			case 270:
				...
				curItem.style.cssText = `;
                    ...
                    transform: 
                        rotateZ(${rotateDeg}deg) 
                        scale3d(${scaleX},${scaleY},1) 
                        translateX(${((mouseY - curItemViewTop - centerX) *
							(scaleX - 1)) /
							scaleX}px) 
                        translateY(${(-(mouseX - curItemViewLeft - centerY) *
							(scaleY - 1)) /
							scaleY}px) 
                    ;
	...
	setToInitialSize(
		...
		switch (rotateDeg % 360) {
			case 0:
				...
				const viewTopInitial: number = Number(
					curItem.dataset.initialTop
				);
				const viewLeftInitial: number = Number(
					curItem.dataset.initialLeft
				);

				let disteanceY: number =
					curItemViewTop +
					centerY * (1 - scaleY) -
					top -
					viewTopInitial;
			...
			case 270:
				{
					...
					let intialItemWidth: number = Number(
						curItem.dataset.initialWidth
					);
					let intialItemHeight: number = Number(
						curItem.dataset.initialHeight
					);
					...
					const viewTopInitial: number =
						(conHeight - intialItemWidth) / 2;
					const viewLeftInitial: number =
						(conWidth - intialItemHeight) / 2;
	...
	handleMove(e: TouchEvent & MouseEvent) {
		...
		if (
			this.touchStartX - curTouchX > 2 &&
			Math.abs(this.touchStartY - curTouchY) > 2
		) {
			clearTimeout(this.performerClick);
		}
		...
		let direction: string =
			e.touches[0].clientX - this.startX > 0 ? 'right' : 'left';
		...
		if (this.fingerDirection) {
			...
			this.performerRecordMove = setTimeout(() => {
				...
				if (curItem.dataset.isEnlargement == 'enlargement') {
					...
					const curItemViewLeft: number = curItem.getBoundingClientRect()
						.left;
					const curItemViewRight: number = curItem.getBoundingClientRect()
						.right;
					// 对于长图单独处理，长图就是宽度可以容纳在当前容器内，但是高度很高的图片
	...
	handleZoom(e: TouchEvent & MouseEvent): void {
		...
		const distaceBefore: number = Math.sqrt(
			Math.pow(this.curPoint1.x - this.curPoint2.x, 2) +
				Math.pow(this.curPoint1.y - this.curPoint2.y, 2)
		);

		const distanceNow: number = Math.sqrt(
		...
		const centerX: number =
			(this.curStartPoint1.x + this.curStartPoint2.x) / 2 - left;
		const centerY: number =
			(this.curStartPoint1.y + this.curStartPoint2.y) / 2 - top;
				...
				case 180:
					{
						...
						curItem.dataset.top = (
							top -
							this.zoomScale * centerY
						).toString();
						curItem.dataset.left = (
							left -
							this.zoomScale * centerX
						).toString();
						curItem.style.cssText += `
	...
	autoMove(
		...
		const curItemViewBottom: number = curItem.getBoundingClientRect()
			.bottom;
		const curItemViewLeft: number = curItem.getBoundingClientRect().left;
		const curItemViewRight: number = curItem.getBoundingClientRect().right;
		deg = (deg / 180) * Math.PI;
	...
	slideNext() {
		...
		let step: number = this.computeStep(
			Math.abs(endX - this.imgContainerMoveX),
			this.slideTime
		);
	...
	pcBeforeClose() {
		document.body.style['overflow'] =
			document.body.dataset['imgPreOverflow'];
	}
	mobileBeforeClose() {}
	show(index: number) {
		this.curIndex = index;
	...
	mobileReadyShow() {}
	pcReadyShow() {
		let styleDesc: CSSStyleDeclaration = window.getComputedStyle(
			document.body
		);
