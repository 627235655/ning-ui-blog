	...
	gesturef = (event) => {
		...
		switch (event.type) {
			case 'gesturestart':
				...
				this.currentPlaceholderRect = this.elPlaceholder.getBoundingClientRect();
				break;
			case 'gesturechange':
				console.log('gesturechange', event.detail.scaleNum);
				if (!event.detail.scaleNum) return;
				let scaleNum =
					(event.detail.scaleNum *
						this.currentPlaceholderRect.width) /
				...
				let scrollLeft =
					scaleNum * rect.width > clientWidth
						? (MAX_SCALE - 1) *
								(transformOriginPoint[0] - rect.left) +
						  rect.left
						: 0;
				let scrollTop =
					scaleNum * rect.height > clientHeight
						? (MAX_SCALE - 1) *
								(transformOriginPoint[1] - rect.top) +
						  rect.top
						: 0;
				scrollTop > 0 &&
					Math.animation(
			...
			case 'gestureend':
				...
				if (this.scaleNum > MAX_SCALE) {
					let top =
						(clientHeight -
							MAX_SCALE * this.defaultPostionCssTextObj.height) /
						2;
					let left = 0;
					let cssTextObj = {
						width: this.defaultPostionCssTextObj.width * MAX_SCALE,
						height:
							this.defaultPostionCssTextObj.height * MAX_SCALE,
						top,
						left,
					};
					this.setPlaceHolderPosition(cssTextObj);
				...
				if (this.scaleNum < MIN_SCALE) {
					let top =
						(clientHeight -
							MIN_SCALE * this.defaultPostionCssTextObj.height) /
						2;
					let left = 0;
					let cssTextObj = {
						width: this.defaultPostionCssTextObj.width * MIN_SCALE,
						height:
							this.defaultPostionCssTextObj.height * MIN_SCALE,
						top,
						left,
					};
					this.setPlaceHolderPosition(cssTextObj);
