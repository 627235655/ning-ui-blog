...
class CommentApp extends Component {
	...
	handleShowCommentInput = (comment) => {
		this.setState({
			show_comment_input:
				comment._id === this.state.show_comment_input
					? true
					: comment._id,
		});
	};
		...
		return (
			<div className='comment-app'>
				...
				{this.state.commentTotalCount > 0 && (
					<h4 className='tc'>
						<span className=''>
							{this.state.commentTotalCount + ' 条评论'}
						</span>
					</h4>
				)}
				<CommentList
				...
				{this.props.comments.map((comment, i) => (
					<Comment
						...
						handleShowCommentInput={
							this.props.handleShowCommentInput
						}
				...
				<div className='flex-row-box m-t-sm _xs _gray'>
					...
					<span className='flex-center-box'>
						...
						<i
							...
							onClick={() =>
								this.props.handleShowCommentInput(comment)
							}
						></i>
					</span>
				</div>
