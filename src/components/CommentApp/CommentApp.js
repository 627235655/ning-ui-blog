...
class CommentApp extends Component {
  ...
  getCommentList = () => {
    ...
    axios
      .get("/api/getCommentList", { params: data })
  ...
  handleShowCommentInput = (comment) => {
    this.setState({
      show_comment_input:
        comment._id === this.state.show_comment_input ? true : comment._id
    });
  };
  ...
  setLikeCount = (comment_id) => {
    ...
    axios
      .post("/api/setCommentLikeCount", data)
      .then(function (response) {
        let res = response.data;
        if (res.status === 200) {
    ...
    return (
      <div className="comment-app">
        ...
        {this.state.commentTotalCount > 0 && (
          <h4 className="tc">
            <span className="">{this.state.commentTotalCount + " 条评论"}</span>
          </h4>
        )}
        <CommentList
          articleName={this.props.articleName}
...
class CommentInput extends Component {
  ...
  render() {
    return (
      <div>
        <h4 className="tc" id="comment_app">
          <span
            className="ning-prompt-trigger"
            data-prompt="{content: '留下你宝贵的评论吧', theme: 'blue'}"
          >
            坐而论道，山高水长
          </span>
        </h4>
        <div className="comment-input ning-row">
          ...
          <div className="ning-form-item col-12">
            <label></label>
            <button className="ning-btn" onClick={() => this.addComment()}>
              发布评论
            </button>
          </div>
        </div>
    ...
    return (
      <div className="m-b-lg">
        ...
        {this.props.comments.length === 0 && (
          <hr className="m-t-lg m-b-lg" text="暂时还没有评论哟，抢占一楼吧~" />
        )}
      </div>
    );
  }
      ...
      <div className="comment">
        <div>
          <span className="_xs _gray">
            {comment.website ? (
              <a
                className={comment.isAuthor ? "_sm _bold blue" : "_sm _bold"}
              ...
              <span className="_sm _bold">{comment.userName}</span>
            )}
            {comment.toUserName
        ...
        <div className="flex-row-box m-t-sm _xs _gray">
          ...
          <span className="flex-center-box">
            <i
              className="ning-icon icon-heart m-r-sm"
              onClick={() => this.props.setLikeCount(comment._id)}
            ></i>
            {comment.likeCount > 0 && `(${comment.likeCount})`}
            <span className="m-l-md m-r-md">|</span>
            <i
              className="ning-icon icon-comment"
              onClick={() => this.props.handleShowCommentInput(comment)}
            ></i>
          </span>
        </div>
