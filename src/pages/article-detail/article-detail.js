...
class ArticleDetail extends Component {
        ...
        <div id="article_detail" className="ning-container p-md">
          <div className="detail-tool">
            <div
              ...
              onClick={(e) => this.addAtricleLikeCount(e)}
            >
              <i className="ning-icon icon-heart"></i>
              {data.likeCount ? (
                <span className="ning-badge">{data.likeCount}</span>
              ) : null}
            </div>
            <a className="detail-tool-item" href="#comment_app">
              <i className="ning-icon icon-comment"></i>
              {data.commentCount ? (
                <span className="ning-badge">{data.commentCount}</span>
              ) : null}
            </a>
            <a
              className="detail-tool-item"
              href="https://weibo.com/5382177688/profile?topnav=1&wvr=6"
              target="_blank"
            >
          ...
          <div className="article-detail-wrap">
            <div
              className="article_preview-header"
              style={{
                backgroundImage: `url(${data.thumbnailUrl})`
              }}
            >
              <h2>
                <span className="article_preview-title">
                  {data.articleName}
                </span>
              </h2>
              <p className="article_preview-summary flex-center-box">
                <span className="tl">{data.articleSummary}</span>
              </p>
              <p className="article_preview-header_footer">
                {data.articleTags.length > 0 && (
                  <span className="article_preview-tags flex-center-box">
                    <i
                      data-prompt="{content: '标签', theme: 'blue'}"
                      className="ning-prompt-trigger ning-icon icon-tag m-r-xs"
                    ></i>
                    :
                ...
                <span className="m-l-md flex-center-box">
                  <i
                    data-prompt="{content: '阅读量', theme: 'blue'}"
                    className="ning-prompt-trigger ning-icon icon-reading m-r-xs"
                  ></i>
                  : {data.readCount || "-"}
                </span>
                <span className="m-l-md flex-center-box">
                  <i
                    data-prompt="{content: '点赞数，thx~', theme: 'blue'}"
                    className="ning-prompt-trigger ning-icon icon-heart m-r-xs"
                  ></i>
                  : {data.likeCount || "-"}
                </span>
                <span className="m-l-md flex-center-box">
                  <i
                    data-prompt="{content: '总字数', theme: 'blue'}"
                    className="ning-prompt-trigger ning-icon icon-count m-r-xs"
                  ></i>
                  : {data.articleContentLength || "-"}
                </span>
                <span className="m-l-md flex-center-box">
                  <i
                    data-prompt="{content: '阅读时长', theme: 'blue'}"
                    className="ning-prompt-trigger ning-icon icon-time m-r-xs"
                  ></i>
                  : {Math.ceil(data.articleContentLength / 400)} min
                </span>
                <span className="m-l-md flex-center-box">
                  <i
                    data-prompt="{content: '发布日期', theme: 'blue'}"
                    className="ning-prompt-trigger ning-icon icon-date m-r-xs"
                  ></i>
                  :{" "}
            ...
            <div
              className="m-b-lg"
              id="articleContentResult"
              dangerouslySetInnerHTML={{
                __html: marked(data.articleContentResult)
              }}
            />
            {this.state.catalogue_list.length > 0 && (
              <ArticleCatalogue catalogue_list={this.state.catalogue_list} />
            )}
            <CopyRight createDate={new Date(data.createDate).Format()} />
            <h4 className="tc m-t-lg m-b-md">
              <span
                className="ning-prompt-trigger"
                data-prompt="{content: '相关文章', theme: 'blue'}"
              >
                他山之石，可以攻玉
              </span>
            </h4>
            <ul className="blog-list ning-row">{el_article_list}</ul>
  ...
  addAtricleReadCount() {
    let data = {
        _id: this.state._id,
        readCount: this.state.articleDetail.readCount
          ? ++this.state.articleDetail.readCount
          : 1
      },
      cb = () => {
  ...
  addAtricleLikeCount(e) {
    ...
    let data = {
        _id: this.state._id,
        likeCount: this.state.articleDetail.likeCount
          ? ++this.state.articleDetail.likeCount
          : 1
      },
      cb = () => {
