...
class Catalogue {
	init = (selector: any) => {
		let node_list: Array<any> = [];
		let h_list: any = document.querySelectorAll(`${selector} h3, ${selector} h4`);
		h_list.length > 0 && h_list.forEach((v: any, i: number) => {
			if (v.nodeName === 'H3') {
				let obj: object = {
...
class ArticleDetail extends React.Component<IProps, IState> {
			...
			return (
				<div id="article_detail" className="ning-container p-md">
					<div className="detail-tool">
						<div className="detail-tool-item" onClick={(e: any) => this.addAtricleLikeCount(e)}>
		                	...
		                	<p className="article_preview-header_footer">
			                	{ data.articleTags.length > 0 &&
				                    <span className="article_preview-tags flex-center-box">
				                        ...
				                        {
				                        	data.articleTags.map((v: any, i: number) => {
				                        		return `【${v}】`
				                        	})
				                    	}
				                    </span>
	...
	getArticleDetail = (is_add_read_count?: any) => {
		let self = this,
			url = server.getArticleDetail,
			data = {
				_id: this.state._id,
			},
			cb = (res: any) => {
