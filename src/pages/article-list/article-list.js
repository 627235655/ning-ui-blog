		...
		if (!tag) {
			...
			if (articleTags.length === 0) {
				...
				this.props.history.replace(
					`/index/article-list?tagName=${articleTags.join(
						URL_PARAMS_GAP
					)}`
				);
			}
	...
	setDateList = (date) => {
		...
		if (!date) {
			...
			if (createDates.length === 0) {
				...
				this.props.history.replace(
					`/index/article-list?createDates=${createDates.join(
						URL_PARAMS_GAP
					)}`
				);
			}
	...
	getArticleList = (currentPage) => {
		...
		if (data.filter.createDates.length > 0) {
			...
			data.filter.createDates.map((v, i) => {
				new_arr.push(
					util.getMonthFirstDay(new Date(v + '-01')).getTime()
				);
			});
		...
		let el_tag_list = this.state.tagList.map((v, i, a) => {
			return (
				<span
					...
					className={
						'tag-item ' +
						(this.state.filter.articleTags.includes(v.tagName)
							? 'active'
							: '')
					}
