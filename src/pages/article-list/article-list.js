...
class ArticleList extends Component {
  constructor(props) {
    super(props);
    let articleTags = props.location.search
    ...
    if (!tag) {
      ...
      if (articleTags.length === 0) {
        ...
        this.props.history.replace(
          `/index/article-list?tagName=${articleTags.join(URL_PARAMS_GAP)}`
        );
      }
    }
  };
  ...
  setDateList = (date) => {
    ...
    if (!date) {
      ...
      if (createDates.length === 0) {
        ...
        this.props.history.replace(
          `/index/article-list?createDates=${createDates.join(URL_PARAMS_GAP)}`
        );
      }
    }
  };
      ...
      .then(function (response) {
        ...
        if (res.status === 200) {
          ...
          self.setState({
            ...
            all_total:
              self.state.filter.articleTags.length === 0 &&
              self.state.filter.createDates.length === 0
                ? res.data.totalCount
                : self.state.all_total
          });
          let params = {
    ...
    let el_tag_list = this.state.tagList.map((v, i, a) => {
      return (
        <span
          ...
          className={
            "tag-item " +
            (this.state.filter.articleTags.includes(v.tagName) ? "active" : "")
          }
          onClick={() => this.setTagList(v.tagName)}
        >
          <span className="num">
            {v.articleNum > 99 ? "99+" : v.articleNum}
          </span>
          <span
            data-prompt={`{content: ${
              summary_config[v.tagName] || ""
            }, theme: 'blue'}`}
            className="txt ning-prompt-trigger"
          >
            {v.tagName}
          </span>
        </span>
      ...
      el_article_list = this.state.articleList.map((v, i, a) => {
        return (
          <li key={v._id} className="col-3 p-md">
            <ArticleItem detail={v} history={this.props.history} />
          </li>
        );
      });
    }
    let el_date_list = this.state.dateList.map((v, i, a) => {
      return (
        <span
          ...
          className={
            "time-item " +
            (this.state.filter.createDates.includes(v.createDate)
              ? "active"
              : "")
          }
          onClick={() => this.setDateList(v.createDate)}
        >
        ...
        <div className="ning-container flex-box p-lg">
          <div className="article-list-filter">
            {this.state.filterType === "tag" && (
              <div>
                <span
                  className={
                    "tag-item " +
                    (this.state.filter.articleTags.length === 0 && "active")
                  }
                  onClick={() => this.setTagList()}
                >
                  <span className="num">
                    {this.state.all_total
                      ? this.state.all_total > 99
                        ? "99+"
                        : this.state.all_total
                      : "++"}
                  </span>
            ...
            {this.state.filterType === "date" && (
              <div>
                <span
                  className={
                    "time-item year " +
                    (this.state.filter.createDates.length === 0 && "active")
                  }
                  onClick={() => this.setDateList()}
                >
                  <i className="ning-icon icon-time"></i>
                  <span className="txt">全部好文</span>
                  <span className="num">
            ...
            <span className="filter-type-handle">
              ...
              <span
                className={
                  "flex-center-box item " +
                  (this.state.filterType === "tag" && "active")
                }
                onClick={() => this.setFilterType("tag")}
              >
              ...
              <span
                className={
                  "flex-center-box item " +
                  (this.state.filterType === "date" && "active")
                }
                onClick={() => this.setFilterType("date")}
              >
          ...
          <ul className="ning-row">{el_article_list}</ul>
          <div
            className="ning-paginator p-l-md p-r-md"
            id="article_list_paginator"
          ></div>
        </div>
      </div>
    );
