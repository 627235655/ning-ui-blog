...
declare global {
    interface Window{
        mozRequestAnimationFrame: () => string;
        oRequestAnimationFrame: () => string;
        msRequestAnimationFrame: () => string;
    }

    interface HTMLElement{
        getContext: (string: string) => any;
        width: number;
        height: number;
    }
}
...
class Profile extends React.Component<{}, IState> {
    ...
    typing = () => {
        ...
        if (this.state.str_index <= this.state.str.length) {
            let str_index = this.state.str_index;
            str_index++;
            this.setState({
                str_index,
            },() => {
    ...
    //             label: {
    //                 offset: 12,
    //                 formatter(text: string, item: string, index: number) {
    ...
    renderCanvas = () => {
        ...
        function resize() {
            ...
            canvas.setAttribute('width', x * 2 + 'px');
            canvas.setAttribute('height', y * 2 + 'px');
            // ctx.scale(2,2)
        }
        var RAF = (function() {
