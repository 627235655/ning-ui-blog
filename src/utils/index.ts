...
/**
 * 分割元数组为长度为 n 的子数组的二维数组
 * @param array 需要分割的数组
 * @param n 子数组长度
 */
export const arraySliceN = (array: Array<any>, n: number) => {
  const carouselList = [];
  const num = 3;
  for (let i = 0; i < array.length; i += num) {
    carouselList.push(array.slice(i, i + num));
  }
  return carouselList;
};

export function debouce(fn: (...args) => void, delay: number = 300) {
  let timer = null;
...
class Util {
  init() {
    ...
    Math.easeout = function (
      A: number,
      B: number,
      rate: number,
      callback: (num: number, bool: boolean) => void
    ) {
  ...
  getPassedTime(old_time) {
    ...
    let gap: number | string = now_time.getTime() - old_time.getTime();
    ...
    if (3600 <= gap && gap < 3600 * 24) {
      gap = Math.ceil((gap as number) / 3600) + "小时前";
    }
    if (60 <= gap && gap < 3600) {
      gap = Math.ceil((gap as number) / 60) + "分钟前";
    }
    if (gap < 60) {
      gap = gap + "秒前";
    }
    ...
    axios({
      ...
      .then(function (res) {
        ...
        if (res.data.status === 200) {
          ...
          notify.warning(
            res.data.message ? res.data.message : "网络或服务异常"
          );
        }
      })
