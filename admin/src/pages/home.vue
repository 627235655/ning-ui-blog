    ...
    <div class="ning-row border p-md">
      <div class="col-12 overview">
        <div class="overview-item">
          <h2 class="m-b-md flex-row-box flex-center-box">
            {{ this.overviewData.dateTotal }} 天<span class="tips">{{
              this.overviewData.dateRange
            }}</span>
          </h2>
          <p>创作时长</p>
        ...
        <div class="overview-item">
          ...
          <p class="flex-row-box">
            ...
            <span>|</span>
            <span
              ><a :href="this.overviewData.maxReadCountUrl"
                >单篇最高阅读</a
              ></span
            >
          </p>
        ...
        <div class="overview-item">
          ...
          <p class="flex-row-box">
            ...
            <span>|</span>
            <span
              ><a :href="this.overviewData.maxLikeCountUrl"
                >单篇最高点赞</a
              ></span
            >
          </p>
        ...
        <div class="overview-item">
          ...
          <p class="flex-row-box">
            ...
            <span>|</span>
            <span
              ><a :href="this.overviewData.maxCommentCountUrl"
                >单篇最高评论</a
              ></span
            >
          </p>
      ...
      <div class="col-6 border p-sm m-b-md">
        <h3 class="p-b-md flex-center-box">
          <i class="ning-icon icon-count m-r-sm"></i>总数统计
        </h3>
        <!-- 文章数, 标签数，总字数 -->
      ...
      <div class="flex-1 border p-sm m-b-md m-l-md">
        <h3 class="p-b-md flex-center-box">
          <i class="ning-icon icon-percentage m-r-sm"></i>标签占比
        </h3>
        <!-- 文章数, 标签数，总字数 -->
      ...
      <div class="col-12 border p-sm">
        <h3 class="p-b-md flex-center-box">
          <i class="ning-icon icon-date m-r-sm"></i>时间活跃度
        </h3>
        <!-- 文章数, 标签数，总字数 -->
...
let initDateChartData = function (length) {
  ...
  let len =
    length || Math.ceil((timestamp - start_day.getTime()) / (3600 * 1000 * 24)); // 得出总天数
  let arr = [];
  for (let i = 0; i < len; i++) {
...
export default {
  ...
  mounted: function () {
    ...
    Promise.all([this.getTagList(), this.getArticleList()])
      .then((result) => {
  ...
  methods: {
    setOverView(totalArticleList) {
      ...
      this.overviewData.dateTotal = totalArticleList.length
        ? (
            (new Date(totalArticleList.pop().createDate).getTime() -
              new Date(totalArticleList[0].createDate).getTime()) /
            (3600 * 24 * 1000)
          ).toFixed(0)
        : 0;
      this.overviewData.dateRange = totalArticleList.length
        ? new Date(totalArticleList[0].createDate).Format('yy-MM-dd') +
          '~' +
          new Date(totalArticleList.pop().createDate).Format('yy-MM-dd')
        : 0;
    ...
    renderDateChart() {
      ...
      chart.axis('week', {
        ...
        label: {
          ...
          formatter: function formatter(val) {
            ...
            let date = new Date(start_time + one_week_time * val).Format(
                'yyyy-MM-dd'
              ),
      ...
      chart
        ...
        .color('articles', '#BAE7FF-#1890FF-#0050B3')
