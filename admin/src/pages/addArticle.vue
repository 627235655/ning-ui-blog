    ...
    <div class="ning-row">
      <div class="ning-form col-6 border" :class="{ active: collapsed }">
        ...
        <div class="ning-form-item flex-allcenter-box">
          ...
          <button
            class="ning-btn _green m-l-md"
            id="form_collapsed_btn"
      ...
      <div class="border flex-1 m-l-md p-md pos-r" id="article_preview">
        ...
        <p class="ning-summary" v-if="data.articleSummary">
          {{ data.articleSummary }}
        </p>
        <div id="articleContentResult" v-highlight></div>
        <p class="count">
          字数统计:<span> {{ data.articleContentLength }} </span>
        </p>
      </div>
      <button class="collapsed-btn" @click="collapsed = !collapsed">
        {{ collapsed ? '继续编辑' : '预览文章' }}
      </button>
    </div>
  </div>
...
const ThumbnailUrl =
  'https://zongyuan.oss-cn-shenzhen.aliyuncs.com/ning-ui-blog/1543218076685.png';
...
export default {
  ...
  methods: {
    renderMarkDown(value, render) {
      ...
      this.el_articleContentResult = document.getElementById(
        'articleContentResult'
      );
      //展示到对应的地方
      ...
      this.data.articleContentLength = util.formatHtmlStr(
        this.data.articleContentResult
      ).length;
    },
    addArticle() {
      let self = this,
